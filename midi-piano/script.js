...
document.addEventListener("keydown", (e) => {
  ...
  const noteDet = getNote(keyCode);

  if (noteDet == null) return;
...
document.addEventListener("keyup", (e) => {
  ...
  noteDet.active = false;
  playNotes();
});

function getNote(keyboardKey) {
  return NOTE_DETAILS.find((noteDet) => `Key${noteDet.key}` === keyboardKey);
}
...
function playNotes() {
  ...
  gain = 1 / activeNotes.length;
  activeNotes.forEach((activeNote) => {
    startNote(activeNote, gain);
  });
}
